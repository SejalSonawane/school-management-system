...
exports.importUnits = async (req, res) => {
  ...
  try {
    ...
    // Read raw to get actual header row
    ...
    // Map of truncated/malformed headers to correct ones
    const headerMap = {
      "type_of_m": "type_of_management",
      "type_of_management": "type_of_management",
      "school_jur": "school_jurisdiction",
      "school_jurisdiction": "school_jurisdiction",
      "competen": "competent_authority_name",
      "competent_authority_name": "competent_authority_name",
      "authority_": "authority_number",
      "authority_number": "authority_number",
      "authority_k": "authority_zone",
      "authority_zone": "authority_zone",
      "kendrasha": "kendrashala_name",
      "kendrashala_name": "kendrashala_name",
      "info_authe": "info_authority_name",
      "info_authority_name": "info_authority_name",
      "appellate_": "appellate_authority_name",
      "appellate_authority_name": "appellate_authority_name",
      "midday_m": "midday_meal_org_name",
      "midday_meal_org_name": "midday_meal_org_name",
      "midday_meal_org_contact": "midday_meal_org_contact",
      "standard_": "standard_range",
      "standard_range": "standard_range",
      "headmistre": "headmistress_name",
      "headmistress_name": "headmistress_name",
      "headmistress_phone": "headmistress_phone",
      "headmistress_email": "headmistress_email",
      "school_shi": "school_shift",
      "school_shift": "school_shift",
      "scholarship": "scholarship_code",
      "scholarship_code": "scholarship_code",
      "first_grant": "first_grant_in_aid_year",
      "first_grant_in_aid_year": "first_grant_in_aid_year",
      "budget_v": "budget_version",
      "budget_version": "budget_version",
      "budget_i": "budget_income",
      "budget_income": "budget_income",
      "budget_ex": "budget_expenses",
      "budget_expenses": "budget_expenses",
      "budget_su": "budget_surplus",
      "budget_surplus": "budget_surplus",
      "case_des": "case_description",
      "case_description": "case_description",
      "bank_nam": "bank_name",
      "bank_name": "bank_name",
      "bank_purp": "bank_purpose",
      "bank_purpose": "bank_purpose",
      "payments_f": "payments_fiscal_year",
      "payments_fiscal_year": "payments_fiscal_year",
      "payments_c": "payments_category",
      "payments_category": "payments_category"
    };
    ...
    console.log("Normalized Excel headers:", rowKeys);
    ...
    if (missingHeaders.length > 0) {
      ...
      // Don't fail on missing headers - try to import anyway
    }
    ...
    for (const row of rows) {
      ...
      if (!unit_id || !semis_no) {
        console.warn("Skipping row with missing unit_id or semis_no:", row);
        continue;
      }

      console.log(`Processing unit: ${unit_id}`);
      ...
      if (hasBudget) {
        await client.query(
          `INSERT INTO unit_budgets
           ...
           VALUES ($1,$2,$3,$4,$5,$6)
           ON CONFLICT (unit_id, fiscal_year, version) DO UPDATE SET
      ...
      if (case_description && String(case_description).trim() !== "") {
        await client.query(
          `INSERT INTO unit_cases (unit_id, description) VALUES ($1,$2)
           ON CONFLICT (unit_id) DO UPDATE SET description = EXCLUDED.description`,
        ...
        await client.query(
          `INSERT INTO unit_banks (unit_id, bank_name, bank_purpose)
           ...
           ON CONFLICT (unit_id) DO UPDATE SET
             ...
             bank_purpose = EXCLUDED.bank_purpose`,
          [
            Number(unit_id),
      ...
      if (hasPayment) {
        await client.query(
          `INSERT INTO unit_payments
           ...
           VALUES ($1,$2,$3,$4)
           ON CONFLICT (unit_id, fiscal_year, category) DO UPDATE SET
    ...
    res.status(200).json({
      message: "Units import completed successfully",
      importedCount: 0
    });
  } finally {
